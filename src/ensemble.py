...
class Ensemble:
    ...
    def pretrain_cpu(self):
        ...
        device = torch.device('cuda') if self.accelerator == 'cuda' else torch.device('cpu')
        ...
        def pretrain_epoch(module, optimizer):
            ...
            for batch_idx, batch in enumerate(train_loader):
                ...
                for i in range(1, len(batch)):
                    loss += nn.functional.huber_loss(input=preds[i - 1].to(device), target=batch[i].view(preds[i - 1].to(device).shape))
                loss.backward()
                optimizer.step()
        ...
        with Parallel(n_jobs=self.devices) as parallel:
            for _ in tqdm(range(self.pretrain_epochs)):
                ...
                for (module, optimizer) in res:
                    self.pretrain_modules.append(module.to(torch.device('cpu')))
                    self.pretrain_optimizers.append(optimizer)
    

    def pretrain_gpu_alt(self):
        train_set = TensorDataset(self.pretrain_configs, *self.pretrain_metrics)
        train_loader = DataLoader(train_set, batch_size=self.pretrain_bs, shuffle=True, num_workers=0)
    ...
    def pretrain_gpu(self):
        ...
        class PretrainEnsemble(nn.Module):
            ...
            def forward(self, x):
                return [mod(x) for mod in self.module_lst]
        ...
        for _ in tqdm(range(self.pretrain_epochs)):
            for batch_idx, batch in enumerate(train_loader):
                optimizer.zero_grad()
                preds_lst = module(batch[0].to(device))
                loss = 0
